...
commands.add_command("spaghetti", "Does spaghett.", function(cmd)
	...
	if player then
		if player ~= nil then
			...
			if not player.admin then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:18')
				return
			end
		else
			p = log
	...
	if param == nil then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:27')
		return
	end
	if param == "true" then
		if not _a.spaghetti_are_you_sure then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:33')
			return
		end
		if _a.spaghetti_enabled == true then
			player.print("Spaghetti is already enabled.", Color.yellow)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:37')
			return
		end
	...
	elseif param == "false" then
		if _a.spaghetti_enabled == false or _a.spaghetti_enabled == nil then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:68')
			return
		end
...
commands.add_command("generate_map", "Pregenerates map.", function(cmd)
	local p_modifer = Modifiers.get_table()
	local _a = p_modifer
	local player = game.player
	...
	if player then
		if player ~= nil then
			...
			if not player.admin then
				p("[ERROR] You're not admin!", Color.fail)
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:111')
				return
			end
		else
			p = log
	...
	if param == nil then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:119')
		return
	end
	if param > 50 then
		player.print("[ERROR] Value is too big.", Color.fail)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:123')
		return
	end
	...
	if not _a.generate_map then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:132')
		return
	end
	local radius = param
	...
	if surface.is_chunk_generated({ radius, radius }) then
		...
		_a.generate_map = nil
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:139')
		return
	end
...
commands.add_command("dump_layout", "Dump the current map-layout.", function()
	local p_modifer = Modifiers.get_table()
	local _a = p_modifer
	local player = game.player
	local p
	...
	if player then
		if player ~= nil then
			...
			if not player.admin then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:161')
				return
			end
		else
			p = log
	...
	if not _a.dump_layout then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:173')
		return
	end
	local surface = game.players[1].surface
...
commands.add_command("creative", "Enables creative_mode.", function()
	local p_modifer = Modifiers.get_table()
	local _a = p_modifer
	local player = game.player
	local p
	...
	if player then
		if player ~= nil then
			...
			if not player.admin then
				p("[ERROR] You're not admin!", Color.fail)
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:230')
				return
			end
		else
			p = log
	...
	if not _a.creative_are_you_sure then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:242')
		return
	end
	if _a.creative_enabled == true then
		player.print("[ERROR] Creative/cheat-mode is already active!", Color.fail)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:246')
		return
	end
	...
	function()
		...
		if player then
			if player ~= nil then
				...
				if not player.admin then
					p("[ERROR] You're not admin!", Color.fail)
					log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/misc.lua:303')
					return
				end
			else
				p = log
