...
local function force_map_reset(reason)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/commands/force_map_reset.lua:3')
	local player = game.player

	if player and player ~= nil then
		if not player.admin then
			player.print("[ERROR] Command is admin-only. Please ask an admin.", Color.warning)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/force_map_reset.lua:9')
			return
		elseif not reason or string.len(reason) <= 5 then
			...
			if not global.rocket_silo["north"].valid then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/commands/force_map_reset.lua:15')
				return
			end
