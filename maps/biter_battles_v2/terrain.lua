...
local function generate_starting_area(pos, surface)
	...
	if distance_from_spawn_wall < 0 then
		if global.random_generator(1, 100) > 23 then
			for _, tree in
				pairs(
					surface.find_entities_filtered({ type = "tree", area = { { pos.x, pos.y }, { pos.x + 1, pos.y + 1 } } })
				)
			do
				tree.destroy()
			end
...
local function mixed_ore(surface, left_top_x, left_top_y)
	...
	if left_top_y == -32 and math_abs(left_top_x) <= 32 then
		for _, e in
			pairs(
				surface.find_entities_filtered({ name = "character", invert = true, area = { { -12, -12 }, { 12, 12 } } })
			)
		do
			e.destroy()
		end
...
function Public.generate_silo(surface)
	...
	for _, t in
		pairs(
			surface.find_tiles_filtered({
				area = { { mirror_position.x - 6, mirror_position.y - 6 }, {
					mirror_position.x + 6,
					mirror_position.y + 6,
				} },
				name = { "water", "deepwater" },
