...
local function clock(frame)
	...
	local clock = frame.add({
		type = "label",
		caption = string.format("Time: %02d:%02d   Speed: %.2f", total_hours, minutes, game.speed),
	})
...
function Public.create_main_gui(player)
	...
	if not is_spec then
		...
		for food_name, tooltip in pairs(food_names) do
			local s =
				t.add({ type = "sprite-button", name = food_name, sprite = "item/" .. food_name, tooltip = tooltip })
		...
		gui_style(
			s,
			{ minimal_height = 41, minimal_width = 41, padding = 0, font_color = { r = 0.9, g = 0.9, b = 0.9 } }
		)
		frame.add({ type = "line" })
	end
					...
					"Not ready to return to your team yet as you are still suspended. Please wait "
						.. math_ceil(
							(
								...
								- (math.floor((game.ticks_played - global.suspended_players[player.name])))
							) / 60
						)
						.. " seconds.",
					{ r = 0.98, g = 0.66, b = 0.22 }
...
function spectate(player, forced_join, stored_position)
	...
	if not forced_join then
		...
		if
			global.active_special_games["captain_mode"] and global.special_games_variables["captain_mode"]["prepaPhase"]
		then
			player.print(
...
local function on_gui_click(event)
	...
	if name == "suspend_no" then
		if global.suspend_voting[player.name] ~= 0 then
			...
			game.print(
				player.name .. " doesn't want to suspend " .. global.suspend_target,
				{ r = 0.9, g = 0.1, b = 0.1 }
			)
		end
	end
