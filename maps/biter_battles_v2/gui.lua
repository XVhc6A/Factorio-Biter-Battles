...
function Public.clear_copy_history(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:46')
...
local function create_sprite_button(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:59')
	if player.gui.top["bb_toggle_button"] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:61')
		return
	end
	local button =
		player.gui.top.add({ type = "sprite-button", name = "bb_toggle_button", sprite = "entity/big-biter" })
	button.style.font = "default-bold"
	element_style({ element = button, x = 38, y = 38, pad = -2 })
end

local function clock(frame)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:69')
	local total_minutes = math.floor(game.ticks_played / (60 * 60))
	local total_hours = math.floor(total_minutes / 60)
	local minutes = total_minutes - (total_hours * 60)

	local clock = frame.add({
		type = "label",
		caption = "Game time: " .. string.format("%02d", total_hours) .. ":" .. string.format("%02d", minutes),
	})
...
local function create_first_join_gui(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:83')
	...
	for _, gui_value in pairs(gui_values) do
		...
		for _, p in pairs(game.forces[gui_value.force].connected_players) do
			...
			l.style.font_color =
				{ r = p.color.r * 0.6 + 0.4, g = p.color.g * 0.6 + 0.4, b = p.color.b * 0.6 + 0.4, a = 1 }
			l.style.font = "heading-2"
		end
...
local function add_tech_button(elem, gui_value)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:143')
...
local function add_prod_button(elem, gui_value)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:154')
...
function Public.create_main_gui(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:164')
	...
	if global.bb_game_won_by_team then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:171')
		return
	end
	if not global.chosen_team[player.name] then
		if not global.tournament_mode then
			create_first_join_gui(player)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:176')
			return
		end
	end
...
function Public.refresh()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:321')
...
function Public.refresh_threat()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:330')
	if global.gui_refresh_delay > game.tick then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:332')
		return
	end
	for _, player in pairs(game.connected_players) do
		if player.gui.left["bb_main_gui"] then
			if player.gui.left["bb_main_gui"].stats_north then
				player.gui.left["bb_main_gui"].stats_north.threat_north.caption =
					math.floor(global.bb_threat["north_biters"])
				player.gui.left["bb_main_gui"].stats_south.threat_south.caption =
					math.floor(global.bb_threat["south_biters"])
			end
		end
	end
	global.gui_refresh_delay = game.tick + 5
...
function join_team(player, force_name, forced_join, auto_join)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:347')
	if not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:349')
		return
	end
	if not forced_join then
		if global.tournament_mode then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:357')
			return
		end
	end
	if not force_name then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:361')
		return
	end
	...
	if not global.training_mode and global.bb_settings.team_balancing then
		if not forced_join then
			if #game.forces[force_name].connected_players > #game.forces[enemy_team].connected_players then
				if not global.chosen_team[player.name] then
					player.print(
						"Team " .. force_name .. " has too many players currently.",
						{ r = 0.98, g = 0.66, b = 0.22 }
					)
					log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:377')
					return
				end
			end
		end
	...
	if global.chosen_team[player.name] then
		if not forced_join then
			if game.tick - global.spectator_rejoin_delay[player.name] < 3600 then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:392')
				return
			end
		end
		...
		if not p then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:403')
			return
		end
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:415')
		return
	end
	local pos =
	...
	if not forced_join then
		...
		if global.tm_custom_name[player.force.name] then
			c = global.tm_custom_name[player.force.name]
		end
		local message = table.concat({ player.name, " has joined team ", c, "! " })
		Server.to_discord_bold(message)
		if auto_join then
			message = table.concat({ player.name, " was automatically assigned to team ", c, "!" })
		end
...
function spectate(player, forced_join)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:454')
	if not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:456')
		return
	end
	if not forced_join then
		if global.tournament_mode then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:464')
			return
		end
	end
...
local function join_gui_click(name, player, auto_join)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:486')
	if not name then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:488')
		return
	end
	...
	if global.game_lobby_active then
		if player.admin then
			...
			global.game_lobby_active = false
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:496')
			return
		end
		player.print(
			"Waiting for more players, " .. wait_messages[math_random(1, #wait_messages)],
			{ r = 0.98, g = 0.66, b = 0.22 }
		)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:502')
		return
	end
	join_team(player, name, false, auto_join)
end
...
function Public.spy_fish()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:508')
...
local function on_gui_click(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:525')
	if not event.element then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:527')
		return
	end
	if not event.element.valid then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:530')
		return
	end
	...
	if name == "bb_toggle_button" then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:540')
		return
	end
	for _, gui_values in pairs(gui_values) do
		if name == gui_values.n1 then
			join_gui_click(gui_values.force, player)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:545')
			return
		end
	end
	...
	if name == "join_random_button" then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:578')
		return
	end

	if name == "raw-fish" then
		Functions.spy_fish(player, event)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:583')
		return
	end

	if food_names[name] then
		feed_the_biters(player, name)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:588')
		return
	end
	...
	if name == "bb_spectate" then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:601')
		return
	end
...
local function on_player_joined_game(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:614')
	local player = game.players[event.player_index]
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/maps/biter_battles_v2/gui.lua:649')
return Public
