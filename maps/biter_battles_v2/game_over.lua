...
function Public.silo_death(event)
	...
	if entity == global.rocket_silo.south or entity == global.rocket_silo.north then
		...
		then
			...
			log_to_db(
				"[RefereeName]" .. global.special_games_variables["captain_mode"]["stats"]["InitialReferee"] .. "\n",
				true
			)
			log_to_db(
				"[CaptainNorth]"
			...
			log_to_db(
				"[StartTick]" .. global.special_games_variables["captain_mode"]["stats"]["tickGameStarting"] .. "\n",
				true
			)
			log_to_db("[WinnerTeam]" .. global.bb_game_won_by_team .. "\n", true)
			log_to_db(
				"[ExtraInfo]" .. global.special_games_variables["captain_mode"]["stats"]["extrainfo"] .. "\n",
				true
			)
			log_to_db(
				"[SpecialEnabled]" .. global.special_games_variables["captain_mode"]["stats"]["specialEnabled"] .. "\n",
				true
			)
				...
				if global.special_games_variables["captain_mode"]["stats"]["playerPlaytimes"][player.name] ~= nil then
					log_to_db(
						"[Playtime]["
							.. player.name
							.. "]"
							.. global.special_games_variables["captain_mode"]["stats"]["playerPlaytimes"][player.name]
							.. "\n",
						true
					)
				end
	...
	if global.reroll_time_left > 0 then
		for _, player in pairs(game.connected_players) do
			if player.gui.top.reroll_frame ~= nil then
				player.gui.top.reroll_frame.reroll_table.children[1].caption = "Reroll map?\t"
					.. global.reroll_time_left
					.. "s"
			end
		end
