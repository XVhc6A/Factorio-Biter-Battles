...
local function spawn_biters(
	...
	for _ = 1, maxLoopIteration, 1 do
		...
		if
			not isItnormalBiters
			and biter_threat - threat_values[unit_name] * 20 * health_buff_equivalent_revive < 0
		then
			break
		...
		if isItnormalBiters and global.biter_spawn_unseen[force_name][unit_name] then
			game.print({
				...
				"A ",
				unit_name:gsub("-", " "),
				" was spotted far away on team ",
				(global.tm_custom_name[force_name] or force_name),
				"...",
			})
		...
		if not isItnormalBiters and global.biter_spawn_unseen[boss_biter_force_name][unit_name] then
			game.print({
				...
				"A ",
				unit_name:gsub("-", " "),
				" boss was spotted far away on team ",
				(global.tm_custom_name[force_name] or force_name),
				"...",
			})
...
function Public.subtract_threat(entity)
	...
	global.bb_threat[biter_not_boss_force] = global.bb_threat[biter_not_boss_force]
		- threat_values[entity.name] * threat_modifier
	return true
end
