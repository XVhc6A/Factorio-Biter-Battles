...
local function on_tick(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/tools/map_pregen.lua:2')
	if global.map_pregeneration_is_active then
		if game.tick % 600 == 0 then
			...
			if
				game.forces.map_pregen.is_chunk_charted(
					game.players[1].surface,
					{ global.chunk_radius, global.chunk_radius }
				)
			then
...
function map_pregen(chunk_radius)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/tools/map_pregen.lua:27')
	...
	game.forces.map_pregen.chart(
		game.players[1].surface,
		{ { x = -1 * radius, y = -1 * radius }, { x = radius, y = radius } }
	)
