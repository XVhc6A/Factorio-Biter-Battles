...
local function clear_gui_captain_mode()
	for _, player in pairs(game.players) do
		if player.gui.center["captain_poll_frame"] then
			player.gui.center["captain_poll_frame"].destroy()
		end
		if player.gui.center["captain_poll_end_frame"] then
			player.gui.center["captain_poll_end_frame"].destroy()
		end
		if player.gui.top["captain_poll_team_ready_frame"] then
			player.gui.top["captain_poll_team_ready_frame"].destroy()
		end
		if player.gui.center["captain_poll_chosen_choice_frame"] then
			player.gui.center["captain_poll_chosen_choice_frame"].destroy()
		end
		if player.gui.center["captain_poll_firstpicker_choice_frame"] then
		...
		if player.gui.top["captain_referee_enable_picking_late_joiners"] then
			player.gui.top["captain_referee_enable_picking_late_joiners"].destroy()
		end
		if player.gui.center["captain_poll_latejoiner_question"] then
			player.gui.center["captain_poll_latejoiner_question"].destroy()
		end
		if player.gui.center["captain_poll_end_latejoiners_referee_frame"] then
			player.gui.center["captain_poll_end_latejoiners_referee_frame"].destroy()
		end
		if player.gui.top["captain_poll_new_joiner_to_current_match"] then
			player.gui.top["captain_poll_new_joiner_to_current_match"].destroy()
		end
		...
		if player.gui.top["captain_manager_toggle_button"] then
			player.gui.top["captain_manager_toggle_button"].destroy()
		end
	end
end
...
local function poll_captain_player(player)
	...
	local frame = player.gui.center.add({
		type = "frame",
		caption = "What do you want to do for the Captain event ?",
		name = "captain_poll_frame",
		direction = "vertical",
	})
	...
	local b = frame.add({
		type = "button",
		name = "captain_no_but_play",
		caption = "I don't want to be captain but I want to play",
	})
	b.style.font_color = Color.green
	b.style.font = "heading-2"
	b.style.minimal_width = 540
	local b =
		frame.add({ type = "button", name = "captain_spectator_only", caption = "I don't want to play but only watch" })
...
local function poll_captain_end_captain(player)
	...
	local frame = player.gui.center.add({
		type = "frame",
		caption = "End poll for players to become captain (beware, need 2 at least to continue!!)",
		name = "captain_poll_end_frame",
		direction = "vertical",
	})
	local b = frame.add({
		type = "button",
		name = "captain_end_captain_choice",
		caption = "End the poll for players to become captain",
	})
	b.style.font_color = Color.green
	b.style.font = "heading-2"
	b.style.minimal_width = 540
	...
	local l =
		frame.add({ type = "label", name = "remainPlayersDidntVote", caption = get_list_players_who_didnt_vote_yet() })
end
...
local function poll_captain_end_late_joiners_referee(player)
	...
	local frame = player.gui.center.add({
		type = "frame",
		caption = "End poll for players to join late",
		name = "captain_poll_end_latejoiners_referee_frame",
		direction = "vertical",
	})
...
local function show_captain_question()
	for _, player in pairs(game.connected_players) do
		...
		table.insert(
			global.special_games_variables["captain_mode"]["listPlayersWhoAreNotNewToCurrentMatch"],
			player.name
		)
...
local function pollGenerator(
	...
	if isItTopFrame then
		if player.gui.top[frameName] then
			player.gui.top[frameName].destroy()
			return
		end
		...
		frame =
			player.gui.center.add({ type = "frame", caption = questionText, name = frameName, direction = "vertical" })
	end
...
local function poll_captain_team_ready(player, isRef)
	...
	if isRef then
		textToShow = "Do you want to force start event without waiting from go from captain?"
	end
	pollGenerator(
		player,
		true,
		nil,
		"captain_poll_team_ready_frame",
		textToShow,
		"Yes",
		"ready_captain_" .. player.name,
		nil,
		nil,
		nil,
		nil
...
local function poll_captain_picking_first(player)
	pollGenerator(
		...
		"The player " .. global.special_games_variables["captain_mode"]["captainList"][1] .. " will pick first",
		"captain_pick_one_in_list_choice",
		"The player " .. global.special_games_variables["captain_mode"]["captainList"][2] .. " will pick first",
		"captain_pick_second_in_list_choice",
		"The captain that will pick first will be chosen randomly",
		"captain_pick_random_in_list_choice"
	)
end
...
local function poll_removing_captain(player)
	pollGenerator(
		player,
		false,
		global.special_games_variables["captain_mode"]["captainList"],
		"captain_poll_chosen_choice_frame",
		"Who should be removed from captain list (popup until 2 captains remains)?",
		"The player Magical1@StringHere wont be a captain",
		"removing_captain_in_list_Magical1@StringHere",
		nil,
		nil,
		nil,
		nil
...
local function poll_alternate_picking(player)
	pickPlayerGenerator(
		...
		"Who do you want to pick ? Bonus pick remaining : " .. get_bonus_picks_amount(player.name),
		"Magical1@StringHere",
...
local function poll_captain_late_joiners(player)
	pollGenerator(
		...
		false,
		nil,
		"captain_poll_latejoiner_question",
		"Do you want to play?",
		"Yes",
		"captain_late_joiner_yes",
		"No",
		"captain_late_joiner_no",
		"No and please dont ask me again for the event (you will be blacklisted from any other late joiners poll for the current captain match only)",
		"captain_late_joiner_no_blacklist"
	)
end

local function generateRendering(
	nameRendering,
	textChosen,
	xPos,
	yPos,
...
local function generateGenericRenderingCaptain()
	...
	generateRendering(
		"captainLineFive",
		"-For admins, as spectator, use ping to talk only to spectators",
		-65,
		y,
		1,
		1,
	...
	generateRendering(
		"captainLineSix",
		"-Teams are locked, if you want to play, ask to be moved to a team",
		-65,
		y,
		1,
		1,
	...
	generateRendering(
		"captainLineSeven",
		"-We are using discord bb for coms (not required), feel free to join to listen ,even if no mic",
		-65,
		y,
		1,
		1,
...
local function update_bonus_picks_enemyCaptain(captainName, valueAdded)
	if global.special_games_variables["captain_mode"]["captainGroupAllowed"] then
		if captainName == global.special_games_variables["captain_mode"]["captainList"][1] then
			global.special_games_variables["captain_mode"]["bonusPickCptTwo"] = global.special_games_variables["captain_mode"]["bonusPickCptTwo"]
				+ valueAdded
		else
			global.special_games_variables["captain_mode"]["bonusPickCptOne"] = global.special_games_variables["captain_mode"]["bonusPickCptOne"]
				+ valueAdded
		end
	end
end
		...
		if captainName == global.special_games_variables["captain_mode"]["captainList"][1] then
			...
			game.print(
				"captain"
					.. captainName
					.. " has now bonus picks : "
					.. global.special_games_variables["captain_mode"]["bonusPickCptOne"]
			)
		else
			global.special_games_variables["captain_mode"]["bonusPickCptTwo"] = global.special_games_variables["captain_mode"]["bonusPickCptTwo"]
				+ valueAdded
			game.print(
				"captain"
					.. captainName
					.. " has now bonus picks : "
					.. global.special_games_variables["captain_mode"]["bonusPickCptTwo"]
			)
		end
	end
end
...
local function auto_pick_all_of_group(cptPlayer, playerName)
	if global.special_games_variables["captain_mode"]["captainGroupAllowed"] then
		...
		for _, player in pairs(game.connected_players) do
			if
				global.chosen_team[player.name] == nil
			...
			then -- only pick player without a team within the same group
				if does_player_wanna_play(player.name) then
					if
						amountPlayersSwitchedForGroup
						< global.special_games_variables["captain_mode"]["groupLimit"] - 1
					then
						...
						game.get_player(player.name).print(
						...
						game.print(
							player.name
								.. " was not picked automatically with group system, as the group limit was reached",
							Color.red
						)
					end
				end
...
local function is_player_in_group_system(playerName)
	...
	if global.special_games_variables["captain_mode"]["captainGroupAllowed"] then
		local playerChecked = game.get_player(playerName)
		if playerChecked.tag == "" then
			return false
		end
		if not startswith(playerChecked.tag, "[cpt") then
			return false
		end
		return true
	else
...
local function generate_captain_mode(
	refereeName,
	autoTrust,
	captainKick,
	pickingMode,
	...
	global.special_games_variables["captain_mode"] = {
		...
		["listPlayers"] = {},
		["listSpectators"] = {},
		["listOfPlayersWhoDidntVoteForRoleYet"] = {},
		["listTeamReadyToPlay"] = {},
		["lateJoiners"] = false,
		["prepaPhase"] = true,
		["countdown"] = 9,
		...
		["firstPick"] = true,
		["blacklistLateJoin"] = {},
		["listPlayersWhoAreNotNewToCurrentMatch"] = {},
	...
	if game.get_player(global.special_games_variables["captain_mode"]["refereeName"]) == nil then
		game.print(
			"Event captain aborted, referee is not a player connected.. Referee name of player was : "
	...
	clear_character_corpses()
	game.print(
		"Captain mode started !! Have fun ! Referee will be "
			.. global.special_games_variables["captain_mode"]["refereeName"]
	)
	if global.special_games_variables["captain_mode"]["autoTrust"] then
	...
	game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).print(
	...
	if global.special_games_variables["captain_mode"]["captainGroupAllowed"] then
		...
		if global.special_games_variables["captain_mode"]["groupLimit"] ~= 0 then
			amountOfPlayers = global.special_games_variables["captain_mode"]["groupLimit"]
		end
	...
	for i = -9, -16, -1 do
		for k = 2, -2, -1 do
			game.surfaces[global.bb_surface_name].set_tiles(
				{ { name = "green-refined-concrete", position = { x = i, y = k } } },
				true
			)
		end
	end
...
local function updateEndPollReferee()
	game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_frame"]["listCpt"].caption =
		get_cpt_list()
	game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_frame"]["listPlayers"].caption =
		get_player_list()
	game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_frame"]["listSpectators"].caption =
		get_spectator_list()
	game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_frame"]["remainPlayersDidntVote"].caption =
		get_list_players_who_didnt_vote_yet()
end

local function updatePollLateJoinReferee()
	if
		game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_latejoiners_referee_frame"]
	then
		game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_latejoiners_referee_frame"]["listPlayers"].caption =
			get_player_list()
		game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_latejoiners_referee_frame"]["listSpectators"].caption =
			get_spectator_list()
		game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_latejoiners_referee_frame"]["remainPlayersDidntVote"].caption =
			get_list_players_who_didnt_vote_yet()
	end
end
...
local function isRefereeACaptain()
	if
		global.special_games_variables["captain_mode"]["captainList"][1]
			== global.special_games_variables["captain_mode"]["refereeName"]
		or global.special_games_variables["captain_mode"]["captainList"][2]
			== global.special_games_variables["captain_mode"]["refereeName"]
	then
		return true
	else
...
local function poll_pickLateJoiners(player)
	pollGenerator(
		...
		true,
		nil,
		"captain_referee_enable_picking_late_joiners",
		"Enable picking phase for late joiners ?",
		"Yes",
		"captain_enabled_late_picking_by_ref" .. player.name,
		nil,
		nil,
		nil,
		nil
...
local function start_captain_event()
	...
	if playerToClear.gui.top["captain_poll_team_ready_frame"] then
		playerToClear.gui.top["captain_poll_team_ready_frame"].destroy()
	end
...
local function group_system_pick(player, playerPicked, captainChosen)
	...
	if get_bonus_picks_amount(player.name) > 0 then
		...
		poll_alternate_picking(
			game.get_player(global.special_games_variables["captain_mode"]["captainList"][captainChosen])
		)
	end
end
...
local function is_captain(playerName)
	if
		global.special_games_variables["captain_mode"]["captainList"][1] == playerName
		or global.special_games_variables["captain_mode"]["captainList"][2] == playerName
	then
		return true
	else
		return false
...
local Public = {
	...
	config = {
		...
		[5] = {
			...
			switch_state = "left",
			allow_none_state = false,
			tooltip = "Picking order at start of event : 1 1 1 1 1 1 1 / 1 2 1 1 1 1 1",
		},
		[6] = {
			name = "captainGroupAllowed",
			type = "switch",
			switch_state = "left",
			allow_none_state = false,
			tooltip = "Groups of players are allowed for picking phase : Yes / No",
		},
		[7] = {
			name = "groupLimit",
			type = "textfield",
			...
			numeric = true,
			width = 40,
			tooltip = "Amount of players max in a group (0 for infinite)",
		},
		[8] = {
			name = "specialEnabled",
			type = "switch",
...
function Public.draw_captain_manager_gui(player)
	...
	textField = t.add({
		name = "captain_remove_playerName",
		type = "textfield",
		text = "ReplaceMe",
		numeric = false,
		width = 140,
	...
	frame.add({
		type = "label",
		caption = "List of players trusted to throw : " .. table.concat(tablePlayerListThrowAllowed, " | "),
	})
	frame.add({ type = "label", caption = "" })
	...
	if
		not global.special_games_variables["captain_mode"]["prepaPhase"]
		and global.special_games_variables["captain_mode"]["captainKick"]
	then
		button = t2.add({
			type = "button",
...
function Public.captain_log_end_time_player(player)
	...
	then
		if global.special_games_variables["captain_mode"]["stats"]["playerSessionStartTimes"][player.name] then
			local sessionTime = game.ticks_played
				- global.special_games_variables["captain_mode"]["stats"]["playerSessionStartTimes"][player.name]
			if global.special_games_variables["captain_mode"]["stats"]["playerPlaytimes"][player.name] then
				global.special_games_variables["captain_mode"]["stats"]["playerPlaytimes"][player.name] = global.special_games_variables["captain_mode"]["stats"]["playerPlaytimes"][player.name]
					+ sessionTime
			else
...
local function on_gui_click(event)
	...
	if element.name == "captain_yes_choice" then
		...
		if player.gui.center["captain_poll_frame"] ~= nil then
			player.gui.center["captain_poll_frame"].destroy()
		end
		...
		game.print(
			"[font=default-large-bold]List of volunteers to become a captain/team picker (total : "
				.. #global.special_games_variables["captain_mode"]["captainList"]
				.. ") : "
				.. listCaptains
				.. "[/font]",
		...
		elseif
			game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_end_frame"]
			~= nil
		then
	...
	elseif element.name == "captain_end_captain_choice" then
		...
		if player.gui.center["captain_poll_end_frame"] ~= nil then
			player.gui.center["captain_poll_end_frame"].destroy()
		end
		...
		elseif #global.special_games_variables["captain_mode"]["captainList"] == 2 then
			...
			game.print(
				"As there are too many players wanting to be captain, referee will pick who will be the 2 captains",
				Color.cyan
			)
	...
	elseif string.find(element.name, "removing_captain_in_list_") then
		...
		player.gui.center["captain_poll_chosen_choice_frame"].destroy()
		game.print(
			"[font=default-large-bold]" .. playerPicked .. " was removed in the captains list[/font]",
			Color.cyan
		)
		...
		elseif #global.special_games_variables["captain_mode"]["captainList"] == 2 then
			game.print(
				"[font=default-large-bold]Only 2 volunteers for captain, no vote needed to elect 2 captains, switching to picking phase ![/font]",
				Color.cyan
			)
			Team_manager.switch_force(global.special_games_variables["captain_mode"]["captainList"][1], "north")
			add_to_trust(global.special_games_variables["captain_mode"]["captainList"][1])
			...
			game.print(
				"[font=default-large-bold]Not enough captains case, event canceled..This case should never happen though, report it to bug[/font]",
				Color.cyan
			)
			force_end_captain_event()
		end
	elseif element.name == "captain_pick_one_in_list_choice" then
		if
			#global.special_games_variables["captain_mode"]["listPlayers"] == 0
			and global.special_games_variables["captain_mode"]["lateJoiners"] == false
		then
			...
			game.get_player(global.special_games_variables["captain_mode"]["refereeName"]).gui.center["captain_poll_firstpicker_choice_frame"]
				.destroy()

			if
				#global.special_games_variables["captain_mode"]["listPlayers"] == 1
				and global.special_games_variables["captain_mode"]["lateJoiners"] == true
			then
				local lastPlayerToSend =
					game.get_player(global.special_games_variables["captain_mode"]["listPlayers"][1])
				...
				poll_alternate_picking(
					game.get_player(global.special_games_variables["captain_mode"]["captainList"][1])
				)
			end
		end
	elseif element.name == "captain_pick_second_in_list_choice" then
		...
		then
			...
			then
				...
				poll_alternate_picking(
					game.get_player(global.special_games_variables["captain_mode"]["captainList"][2])
				)
			end
		end
	elseif element.name == "captain_pick_random_in_list_choice" then
		...
		then
			...
			game.print(
				"Fortune has chosen that "
					.. global.special_games_variables["captain_mode"]["captainList"][captainChosen]
					.. " will pick first",
				Color.cyan
			)
			global.special_games_variables["captain_mode"]["pickingPhase"] = true
			...
			then
				local lastPlayerToSend =
					game.get_player(global.special_games_variables["captain_mode"]["listPlayers"][1])
				local captainForceName = "north"
				if captainChosen == 2 then
					captainForceName = "south"
				end
				game.print(lastPlayerToSend.name .. " was automatically picked")
				switchTeamOfPlayer(lastPlayerToSend.name, captainForceName)
				lastPlayerToSend.print(
	...
	elseif string.find(element.name, "captain_player_picked_") then
		...
		game.get_player(playerPicked).print(
			"Remember to join your team channel voice on discord of free biterbattles (discord link can be found on biterbattles.org website) if possible (even if no mic, it's fine, to just listen, it's not required though but better if you do !)",
			Color.cyan
		)
		local index = {}
		...
		if are_all_players_picked() then
			...
			if not global.special_games_variables["captain_mode"]["lateJoiners"] then
				...
				captainTwo.print(
					"As a captain, you can handle your team by clicking on 'Manage your team' button top of screen",
					{ r = 1, g = 1, b = 0 }
				)
			...
			if
				player.name == game.get_player(global.special_games_variables["captain_mode"]["captainList"][1]).name
			then
				if
					not global.special_games_variables["captain_mode"]["pickingModeAlternateBasic"]
					and global.special_games_variables["captain_mode"]["firstPick"]
				then
	...
	elseif string.find(element.name, "ready_captain_") then
		if game.ticks_played < global.difficulty_votes_timeout then
			player.print(
				"[font=default-large-bold]Wait for end of difficulty vote poll before telling your team is ready (meanwhile, your team should strategize!)[/font]",
				Color.red
			)
			return
		end
		...
		if player.name == refereeName and not isRefereeACaptain() then
			game.print(
				"[font=default-large-bold]Referee " .. refereeName .. " force started the game ![/font]",
				Color.cyan
			)
			...
			if #global.special_games_variables["captain_mode"]["listTeamReadyToPlay"] >= 2 then
				if game.get_player(refereeName).gui.top["captain_poll_team_ready_frame"] then
					game.get_player(refereeName).gui.top["captain_poll_team_ready_frame"].destroy()
				end
				prepare_start_captain_event()
			elseif
				isRefereeACaptain() and not game.get_player(refereeName).gui.top["captain_poll_team_ready_frame"]
			then
				poll_captain_team_ready(
					game.get_player(global.special_games_variables["captain_mode"]["refereeName"]),
					true
				)
			end
		end
	...
	elseif element.name == "captain_yes_wanna_play_new_to_current_match" then
		...
		refPlayer.print(
			"Dear referee, the player " .. player.name .. " would like to play, please send him to a team",
			Color.red
		)
	...
	elseif element.name == "captain_no_wanna_play_new_to_current_match" then
		player.print(
			"Enjoy the match then ! Don't hesitate to ask if you want a bit later to join a team and have fun",
			Color.cyan
		)
		if player.gui.top["captain_poll_new_joiner_to_current_match"] then
			player.gui.top["captain_poll_new_joiner_to_current_match"].destroy()
	...
	elseif element.name == "captain_toggle_throw_science" then
		if global.special_games_variables["captain_mode"]["captainList"][2] == player.name then
			global.special_games_variables["captain_mode"]["southEnabledScienceThrow"] =
				not global.special_games_variables["captain_mode"]["southEnabledScienceThrow"]
			game.forces["south"].print(
				"Can anyone throw science in your team ? "
					.. tostring(global.special_games_variables["captain_mode"]["southEnabledScienceThrow"]),
				{ r = 1, g = 1, b = 0 }
			)
		else
			global.special_games_variables["captain_mode"]["northEnabledScienceThrow"] =
				not global.special_games_variables["captain_mode"]["northEnabledScienceThrow"]
			game.forces["north"].print(
				"Can anyone throw science in your team ? "
					.. tostring(global.special_games_variables["captain_mode"]["northEnabledScienceThrow"]),
				{ r = 1, g = 1, b = 0 }
			)
		end
	...
	elseif element.name == "captain_add_someone_to_throw_trustlist" then
		local playerNameUpdateText =
			player.gui.center["captain_manager_gui"]["captain_manager_root_table"]["captain_add_playerName"].text
	...
	elseif element.name == "captain_remove_someone_to_throw_trustlist" then
		local playerNameUpdateText =
			player.gui.center["captain_manager_gui"]["captain_manager_root_table"]["captain_remove_playerName"].text
		if playerNameUpdateText and playerNameUpdateText ~= "" then
			local tableToUpdate = global.special_games_variables["captain_mode"]["northThrowPlayersListAllowed"]
			local forceForPrint = "north"
			if player.name == global.special_games_variables["captain_mode"]["captainList"][2] then
	...
	elseif element.name == "captain_eject_player" then
		local victim = game.get_player(
			player.gui.center["captain_manager_gui"]["captain_manager_root_table_two"]["captain_eject_playerName"].text
		)
		if victim and victim.valid then
			...
			if victim.force.name == "spectator" then
				return player.print("You cant use this command on a spectator.", Color.red)
			end
			if victim.force.name ~= player.force.name then
				return player.print("You cant use this command on a player of enemy team.", Color.red)
			end
			if not victim.connected then
				return player.print("You can only use this command on a connected player.", Color.red)
			end
			game.print(
				"Captain " .. player.name .. " has decided that " .. victim.name .. " must not be in the team anymore."
			)
...
local function changeCaptain(cmd, isItForNorth)
	...
	if global.special_games_variables["captain_mode"]["prepaPhase"] then
		return playerOfCommand.print(
			"This command is only allowed when prepa phase of event is over, wait for it to start",
			Color.red
		)
	end
	...
	if
		global.special_games_variables["captain_mode"]["captainList"][1] == nil
		or global.special_games_variables["captain_mode"]["captainList"][2] == nil
	then
			...
			if isItForNorth then
				...
				game.print(
					playerOfCommand.name
						.. " has decided that "
						.. victim.name
						.. " will be the new captain instead of "
						.. global.special_games_variables["captain_mode"]["captainList"][1],
					Color.cyan
				)
				local oldCaptain = game.get_player(global.special_games_variables["captain_mode"]["captainList"][1])
				if oldCaptain.gui.center["captain_manager_gui"] then
					oldCaptain.gui.center["captain_manager_gui"].destroy()
				end
				if oldCaptain.gui.top["captain_manager_toggle_button"] then
					oldCaptain.gui.top["captain_manager_toggle_button"].destroy()
				end
...
commands.add_command("replaceReferee", "Admin or referee can decide to change the referee", function(cmd)
	...
	if cmd.parameter then
		...
		if victim and victim.valid then
			...
			local refPlayer = game.get_player(global.special_games_variables["captain_mode"]["refereeName"])
			if refPlayer.gui.top["captain_referee_enable_picking_late_joiners"] then
				refPlayer.gui.top["captain_referee_enable_picking_late_joiners"].destroy()
			end
			if refPlayer.gui.center["captain_poll_end_latejoiners_referee_frame"] then
				refPlayer.gui.center["captain_poll_end_latejoiners_referee_frame"].destroy()
			end
			...
			game.print(
				playerOfCommand.name
					.. " has decided that "
					.. victim.name
					.. " will be the new referee instead of "
					.. global.special_games_variables["captain_mode"]["refereeName"],
				Color.cyan
			)
...
commands.add_command("captainDisablePicking", "Admin or referee can decide to change the referee", function(cmd)
	if not cmd.player_index then
		return
	end
	local playerOfCommand = game.get_player(cmd.player_index)
	...
	global.tournament_mode = false
	game.print(
		"Players are now free to join whatever team they want (tournament mode disabled), choice made by "
			.. playerOfCommand.name,
		Color.green
	)
end)
...
local function on_player_joined_game(event)
	...
	if
		global.special_games_variables["captain_mode"] ~= nil
		and not global.special_games_variables["captain_mode"]["listPlayersWhoAreNotNewToCurrentMatch"][player.name]
	then
		table.insert(
			global.special_games_variables["captain_mode"]["listPlayersWhoAreNotNewToCurrentMatch"],
			player.name
		...
		pollGenerator(
			...
			true,
			nil,
			"captain_poll_new_joiner_to_current_match",
			"Do you want to play in the current match of captain event (no signup required) ? You can also always ask later to be sent to a team if you prefer.",
			"Yes",
			"captain_yes_wanna_play_new_to_current_match",
			"No",
			"captain_no_wanna_play_new_to_current_match",
			nil,
			nil
		)
	end
