...
local function place_entity(surface, position)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:44')
	if math_random(1, 3) ~= 1 then
		surface.create_entity({
			name = rock_raffle[math_random(1, size_of_rock_raffle)],
			position = position,
			force = "neutral",
		})
	end
...
local function is_scrap_area(noise)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:54')
	if noise > 0.75 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:56')
		return
	end
	if noise < -0.75 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:59')
		return
	end
	if noise > 0.12 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:62')
		return true
	end
	if noise < -0.12 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:65')
		return true
	end
end

local function move_away_things(surface, area)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:69')
	for _, e in
		pairs(surface.find_entities_filtered({ type = { "unit-spawner", "turret", "unit", "tree" }, area = area }))
	do
...
local function on_player_mined_entity(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:83')
	local entity = event.entity
	if not entity.valid then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:86')
		return
	end
	if entity.type ~= "simple-entity" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:89')
		return
	end
...
local function on_entity_died(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:103')
	if not event.entity.valid then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:105')
		return
	end
...
local function on_chunk_generated(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:110')
		...
		for y = 0, 31, 1 do
			...
			if not get_tile(position).collides_with("resource-layer") then
				...
				if is_scrap_area(noise) then
					set_tiles(
						{ { name = "dirt-" .. math_floor(math_abs(noise) * 12) % 4 + 3, position = position } },
						true
					)
...
local function on_player_joined_game(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:138')
...
local function on_init()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/terrain_layouts/caves.lua:145')
