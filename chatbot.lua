...
local function on_player_created(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:69')
...
commands.add_command("trust", "Promotes a player to trusted!", function(cmd)
	...
	if player and player.valid then
		if not player.admin then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:84')
			return
		end

		if cmd.parameter == nil then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:88')
			return
		end
		...
		if target_player then
			if trusted[target_player.name] then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:94')
				return
			end
		...
		if cmd.parameter == nil then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:109')
			return
		end
		local target_player = game.players[cmd.parameter]
		if target_player then
			if trusted[target_player.name] == true then
				game.print(target_player.name .. " is already trusted!")
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:115')
				return
			end
			trusted[target_player.name] = true
			game.print(target_player.name .. " is now a trusted player.", { r = 0.22, g = 0.99, b = 0.99 })
	...
	if player then
		if player ~= nil then
			...
			if not player.admin then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:133')
				return
			end
		else
			p = log
		...
		if cmd.parameter == nil then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:140')
			return
		end
		local target_player = game.players[cmd.parameter]
		if target_player then
			if trusted[target_player.name] == false then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:146')
				return
			end
		...
		if cmd.parameter == nil then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:161')
			return
		end
		local target_player = game.players[cmd.parameter]
		if target_player then
			if trusted[target_player.name] == false then
				game.print(target_player.name .. " is already untrusted!")
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:167')
				return
			end
			trusted[target_player.name] = false
			game.print(target_player.name .. " is now untrusted.", { r = 0.22, g = 0.99, b = 0.99 })
...
local function process_bot_answers(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:175')
	local player = game.players[event.player_index]
	...
	for word in string.gmatch(message, "%g+") do
		if links[word] then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:184')
			return
		end
	end
end
...
local function on_console_chat(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:189')
	if not event.player_index then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:191')
		return
	end
...
local function on_console_command(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:197')
	local cmd = event.command
	if not event.player_index then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:200')
		return
	end
	local player = game.players[event.player_index]
	...
	if (cmd == "shout" or cmd == "s") and player and param then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:215')
		return
	elseif not player.admin or not commands[cmd] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:217')
		return
	end
	...
	if player then
		...
		if param then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:246')
			return
		else
			print(player.name .. " ran: " .. cmd .. server_time)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:249')
			return
		end
	else
		if param then
			print("ran: " .. cmd .. ' "' .. param .. '" ' .. server_time)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:254')
			return
		else
			print("ran: " .. cmd .. server_time)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/chatbot.lua:257')
			return
		end
	end
end
