...
function Public.open_debug(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/main_view.lua:22')
	for i = 1, #pages do
		local page = pages[i]
	...
	if frame then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/main_view.lua:34')
		return
	end

	frame =
		screen.add({ type = "frame", name = main_frame_name, caption = "Debuggertron 3003", direction = "vertical" })
	...
	for i = 1, #pages do
		local page = pages[i]
		local tab_button =
			tab_flow.add({ type = "flow" }).add({ type = "button", name = tab_name, caption = page.name })
...
Gui.on_click(tab_name, function(event)
	...
	if selected_index == index then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/main_view.lua:82')
		return
	end
...
Gui.on_click(close_name, function(event)
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/main_view.lua:104')
return Public
