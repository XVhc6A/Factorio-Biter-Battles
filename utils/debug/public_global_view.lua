...
function Public.show(container)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/public_global_view.lua:20')
	...
	for token_id, token_name in pairs(Global.names) do
		local header =
			left_panel.add({ type = "flow" }).add({ type = "label", name = header_name, caption = token_name })
	...
	local refresh_button = right_top_flow.add({
		type = "sprite-button",
		name = refresh_name,
...
Gui.on_click(header_name, function(event)
...
local function update_dump(text_input, data, player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/public_global_view.lua:99')
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/public_global_view.lua:125')
return Public
