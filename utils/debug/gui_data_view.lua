...
local function draw_player_headers(player_panel, selected_index)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:20')
	...
	for player_index, values in pairs(Gui.data()) do
		...
		local header = player_panel.add({ type = "flow" }).add({
			type = "label",
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:44')
	return selected_header
end

function Public.show(container)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:47')
	...
	local refresh_button = right_middle_flow.add({
		type = "sprite-button",
		name = refresh_name,
...
local function draw_element_headers(element_panel, values, selected_index)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:108')
	local copy = {}
	...
	for ei, stored_data in pairs(copy) do
		...
		local middle_header = element_panel.add({ type = "flow" }).add({
			type = "label",
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:149')
	return selected_header
end

Gui.on_click(player_header_name, function(event)
	...
	if not values then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:176')
		return
	end
...
Gui.on_click(element_header_name, function(event)
	local element = event.element
	local header_data = Gui.get_data(element)
...
local function update_dump(text_input, data, player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:215')
...
Gui.on_text_changed(input_text_box_name, function(event)
	local element = event.element
...
Gui.on_click(refresh_name, function(event)
	local element = event.element
	local data = Gui.get_data(element)
	...
	if selected_player_header then
		...
		if selected_element_header then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:260')
			return
		end
	end
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/utils/debug/gui_data_view.lua:268')
return Public
