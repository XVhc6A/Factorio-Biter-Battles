local function coord_string(x, y)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:0')
	local str = tostring(x) .. "_"
	str = str .. tostring(y)
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:3')
	return str
end

local function draw_cell(surface, cell, cell_size, wall_entity_name, force_name)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:6')
	local r = math.floor(cell_size * 0.5)
	local entities = {}
	if cell.north then
		for x = r * -1, r, 1 do
			entities[#entities + 1] =
				{ position = { cell.position.x + x, cell.position.y - r }, name = wall_entity_name, force = force_name }
		end
	end
	if cell.south then
		for x = r * -1, r, 1 do
	...
	if cell.east then
		for y = r * -1, r, 1 do
			entities[#entities + 1] =
				{ position = { cell.position.x + r, cell.position.y + y }, name = wall_entity_name, force = force_name }
		end
	end
	if cell.west then
		for y = r * -1, r, 1 do
...
local function draw_maze(surface, position, size, cell_size, wall_entity_name, force_name)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:40')
...
local function get_random_occupied_cell()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:46')
	...
	if not occupied_cells[1] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:56')
		return
	end
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:58')
	return occupied_cells[math.random(1, #occupied_cells)]
end

local function set_dead_cells()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:61')
	local directions = {
		{ 0, -1 },
		{ 0, 1 },
		{ 1, 0 },
	...
	for c, cell in pairs(maze_cells) do
		if not cell.dead then
			...
			for i = 1, 4, 1 do
				local cell_string =
					coord_string(cell.cell_position.x + directions[i][1], cell.cell_position.y + directions[i][2])
...
local function expand_cell(current_cell)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:86')
	...
	for i = 1, 4, 1 do
		...
		local cell_canditate =
			coord_string(current_cell.cell_position.x + direction[1], current_cell.cell_position.y + direction[2])
		if maze_cells[cell_canditate] then
			if not maze_cells[cell_canditate].occupied then
				...
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:103')
				return maze_cells[cell_canditate]
			end
		end
	end
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:107')
	return false
end

local function expand(size)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:110')
...
local function disable_borders(size)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:124')
...
function create_maze(surface, position, size, cell_size, wall_entity_name, force_name, borderless)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:133')
	if not surface then
		game.print("No surface given.")
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:136')
		return
	end
	if not position then
		game.print("No position given.")
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:140')
		return
	end
	if not size then
		game.print("No size given.")
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:144')
		return
	end
	if not cell_size then
		game.print("No cell_size given.")
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:148')
		return
	end
	if not wall_entity_name then
		game.print("No wall_entity_name given.")
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/maze.lua:152')
		return
	end
