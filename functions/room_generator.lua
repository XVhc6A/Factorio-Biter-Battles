...
local function build_room_rect(surface, position, vector, room_center_position, room_size, offset)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:9')
	local room = {}
	...
	room.entrance_tile = surface.get_tile({
		position.x + offset.x + vector[1] * (room_spacing + 1),
		position.y + offset.y + vector[2] * (room_spacing + 1),
	})
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:49')
	return room
end

local function scan_area_empty(surface, search_area)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:52')
	local tiles = surface.find_tiles_filtered({ area = search_area })
	for _, tile in pairs(tiles) do
		if not tile.collides_with("resource-layer") then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:56')
			return false
		end
	end
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:59')
	return true
end

local function scan_strip_empty(surface, position, vector, length)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:62')
	for d = 0, length, 1 do
		local p = { position.x + vector[1] * d, position.y + vector[2] * d }
		local tile = surface.get_tile(p)
		if not tile.collides_with("resource-layer") then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:67')
			return false
		end
	end
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:70')
	return true
end
...
local function scan_direction_full(surface, position, vector, room_max, room_min)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:74')
	...
	if not scan_area_empty(surface, search_area) then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:86')
		return { x = 0, y = 0 }
	end
	...
	if vector[1] == 0 then
		...
		repeat
			if not x_end then
				if
					not scan_strip_empty(
						surface,
						{ x = position.x + best.x + room_spacing + 1, y = yy },
						vector,
						best.y * 2 + room_spacing2
					)
					or not scan_strip_empty(
						surface,
						{ x = position.x - best.x - room_spacing - 1, y = yy },
						vector,
						best.y * 2 + room_spacing2
			...
			if not y_end then
				...
				if
					not scan_strip_empty(
						surface,
						{ x = xx, y = position.y + vector[2] * (best.y * 2 + room_spacing2 + 1) },
						{ 1, 0 },
						best.x * 2 + room_spacing2
					)
					or not scan_strip_empty(
						surface,
						{ x = xx, y = position.y + vector[2] * (best.y * 2 + room_spacing2 + 2) },
						{ 1, 0 },
						best.x * 2 + room_spacing2
					)
				then
		...
		repeat
			if not y_end then
				if
					not scan_strip_empty(
						surface,
						{ x = xx, y = position.y + best.y + room_spacing + 1 },
						vector,
						best.x * 2 + room_spacing2
					)
					or not scan_strip_empty(
			...
			if not x_end then
				...
				if
					not scan_strip_empty(
						surface,
						{ x = position.x + vector[1] * (best.x * 2 + room_spacing2 + 1), y = yy },
						{ 0, 1 },
						best.y * 2 + room_spacing2
					)
					or not scan_strip_empty(
						surface,
						{ x = position.x + vector[1] * (best.x * 2 + room_spacing2 + 2), y = yy },
						{ 0, 1 },
						best.y * 2 + room_spacing2
					)
				then
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:197')
	return best
end
...
local function get_room_tiles_wiggle(surface, position, size, shape)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:201')
	...
	for _, v in pairs(vectors) do
		...
		if full.x > 0 then
			...
			local room_center_position = {
				x = new_pos.x + v[1] * (full.x + room_spacing + 1),
				y = new_pos.y + v[2] * (full.y + room_spacing + 1),
			}
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:261')
...
local function expand_path_tiles_width(surface, room)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:266')
	if not room then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:268')
		return
	end
	...
	for _, v in pairs(vectors) do
		...
		if not tile.collides_with("resource-layer") then
			entrance_tile = tile
			exit_tile = surface.get_tile({
				path_tiles[#path_tiles].position.x + v[1] * -1,
				path_tiles[#path_tiles].position.y + v[2] * -1,
			})
			if v[1] == 0 then
	...
	if not entrance_tile then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:306')
		return
	end
	...
	if #expansion_vectors == 0 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:316')
		return
	end
	...
	if #expansion_vectors == 0 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:329')
		return
	end
...
local function is_bridge_valid(surface, vector, room)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:353')
	...
	for _, tile in pairs(bridge_tiles) do
		for d = -5, 5, 1 do
			...
			local tile = surface.get_tile(p)
			if not tile.collides_with("resource-layer") then
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:367')
				return
			end
		end
	end

	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:372')
	return true
end

local function build_bridge(surface, position)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:375')
	if math_random(1, 8) == 1 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:377')
		return
	end
	...
	for _, v in pairs(vectors) do
		...
		if room.path_tiles[1] then
			if is_bridge_valid(surface, v, room) then
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:404')
				return room
			else
				room.path_tiles = {}
			end
...
function Public.get_room(surface, position, shape)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:412')
	if not shape then
		shape = "square"
	end
	...
	if room then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:422')
		return room
	end
	...
	if room then
		expand_path_tiles_width(surface, room)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:428')
		return room
	end
end

function Public.draw_random_room(surface, position, shape)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:432')
	if not shape then
		shape = "square"
	end
	...
	if not room then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:439')
		return
	end
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/functions/room_generator.lua:458')
return Public
