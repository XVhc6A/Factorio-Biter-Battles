...
local function shuffle(tbl)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:48')
	local size = #tbl
	for i = size, 1, -1 do
		local rand = math.random(size)
		tbl[i], tbl[rand] = tbl[rand], tbl[i]
	end
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:54')
	return tbl
end

local function process_explosion_tile(pos, explosion_index, current_radius)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:57')
	...
	if tile.name == "out-of-map" then
		...
		global.fluid_explosion_schedule[explosion_index].damage_remaining = global.fluid_explosion_schedule[explosion_index].damage_remaining
			- out_of_map_tile_health
	else
		...
		if entity.valid then
			if entity.health then
				if entity.health <= global.fluid_explosion_schedule[explosion_index].damage_remaining then
					...
					entity.damage(
						global.fluid_explosion_schedule[explosion_index].damage_remaining,
						"player",
						"explosion"
					)
	...
	if global.fluid_explosion_schedule[explosion_index].damage_remaining <= 0 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:122')
		return false
	end

	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:125')
	return true
end

local function create_explosion_schedule(entity)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:128')
	local explosives_amount = math.floor(entity.fluidbox[1].amount)

	if explosives_amount < 1 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:132')
		return
	end
	...
	global.fluid_explosion_schedule[#global.fluid_explosion_schedule].damage_remaining = fluid_damages[entity.fluidbox[1].name]
		* explosives_amount
	...
	for current_radius = 1, 16, 1 do
		...
		global.fluid_explosion_schedule[#global.fluid_explosion_schedule][current_radius].trigger_tick = game.tick
			+ (current_radius * 8)
		...
		for index, tile_position in pairs(circle_coords) do
			...
			global.fluid_explosion_schedule[#global.fluid_explosion_schedule][current_radius][index] =
				{ x = pos.x, y = pos.y }
		end
	end
	entity.die("player")
...
local function on_entity_damaged(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:944')
	local entity = event.entity
	if not entity.health then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:947')
		return
	end
	if not container_types[entity.type] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:950')
		return
	end
	if not entity.fluidbox[1] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:953')
		return
	end
	if not fluid_damages[entity.fluidbox[1].name] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:956')
		return
	end
	if entity.health > entity.prototype.max_health * 0.75 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:959')
		return
	end

	if math.random(1, 3) == 1 or entity.health <= 0 then
		create_explosion_schedule(entity)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:964')
		return
	end
end

local function on_tick(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/fluids_are_explosive.lua:968')
				...
				for radius = 1, #global.fluid_explosion_schedule[explosion_index], 1 do
					if global.fluid_explosion_schedule[explosion_index][radius].trigger_tick == tick then
						for tile_index = 1, #global.fluid_explosion_schedule[explosion_index][radius], 1 do
							local continue_explosion = process_explosion_tile(
								global.fluid_explosion_schedule[explosion_index][radius][tile_index],
								explosion_index,
								radius
							)
