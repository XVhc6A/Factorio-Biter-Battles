...
function Public.draw_gui_char_button(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:26')
	if player.gui.top[draw_main_frame_name] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:28')
		return
	end
	local b =
...
function Public.update_char_button(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:40')
...
local function get_class(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:52')
	local rpg_t = RPG.get("rpg_t")
	local average = (
		rpg_t[player.index].strength
		+ rpg_t[player.index].magicka
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:71')
	return classes[high_attribute_name]
end

local function add_gui_description(element, value, width, tooltip, min_height, max_height)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:74')
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:86')
	return e
end

local function add_gui_stat(element, value, width, tooltip, name, color)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:89')
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:100')
	return e
end

local function add_elem_stat(element, value, width, height, font, tooltip, name, color)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:103')
	local e = element.add({ type = "sprite-button", name = name or nil, caption = value })
	e.tooltip = tooltip or ""
	e.style.maximal_width = width
	e.style.minimal_width = width
	...
	e.style.vertical_align = "center"
	e.style.font_color = color or { 222, 222, 222 }
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:114')
	return e
end

local function add_gui_increase_stat(element, name, player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:117')
	local rpg_t = RPG.get("rpg_t")
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:137')
	return e
end

local function add_separator(element, width)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:140')
	local e = element.add({ type = "line" })
	e.style.maximal_width = width
	e.style.minimal_width = width
	e.style.minimal_height = 12
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:145')
	return e
end

local function remove_settings_frame(settings_frame)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:148')
	Gui.remove_data_recursively(settings_frame)
	settings_frame.destroy()
end

local function remove_main_frame(main_frame, screen)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:153')
...
local function draw_main_frame(player, location)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:163')
	if not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:165')
		return
	end
	...
	local scroll_pane = inside_table.add({
		type = "scroll-pane",
	...
	local mining_speed_value = math.round(
		(player.force.manual_mining_speed_modifier + player.character_mining_speed_modifier + 1) * 100
	) .. "%"
	...
	local crafting_speed_value = math.round(
	...
	local running_speed_value = math.round(
...
function Public.draw_level_text(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:441')
	if not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:443')
		return
	end
	...
	if #players == 0 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:460')
		return
	end
...
function Public.update_player_stats(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:482')
	local rpg_extra = RPG.get("rpg_extra")
	local rpg_t = RPG.get("rpg_t")
	...
	player_modifiers[player.index].character_maximum_following_robot_count_bonus["rpg"] =
		math.round(strength / 2 * 0.03, 3)
...
function Public.toggle(player, recreate)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:516')
	...
	if recreate and main_frame then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:524')
		return
	end
	if main_frame then
		remove_main_frame(main_frame, screen)
...
function Public.remove_frame(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:535')
	local screen = player.gui.screen
	local main_frame = screen[main_frame_name]
...
Gui.on_click(draw_main_frame_name, function(event)
	local player = event.player
	if not player or not player.valid or not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:551')
		return
	end
...
Gui.on_click(save_button_name, function(event)
	local player = event.player
	if not player or not player.valid or not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:560')
		return
	end
...
Gui.on_click(discard_button_name, function(event)
	local player = event.player
	local screen = player.gui.screen
	local frame = screen[settings_frame_name]
	if not player or not player.valid or not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:690')
		return
	end
	if frame and frame.valid then
		frame.destroy()
...
Gui.on_click(settings_button_name, function(event)
	...
	local frame = screen[settings_frame_name]
	if not player or not player.valid or not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:702')
		return
	end

	local surface_name = RPG.get("rpg_extra").surface_name
	if sub(player.surface.name, 0, #surface_name) ~= surface_name then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:707')
		return
	end
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/gui.lua:717')
return Public
