...
local function create_input_element(frame, type, value, items, index)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/settings.lua:11')
	if type == "slider" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/settings.lua:13')
		return frame.add({ type = "slider", value = value, minimum_value = 0, maximum_value = 1 })
	end
	if type == "boolean" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/settings.lua:16')
		return frame.add({ type = "checkbox", state = value })
	end
	if type == "dropdown" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/settings.lua:19')
		return frame.add({ type = "drop-down", items = items, selected_index = index })
	end
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/settings.lua:21')
	return frame.add({ type = "text-box", text = value })
end

function Public.extra_settings(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/settings.lua:24')
	...
	if rpg_extra.enable_health_and_mana_bars then
		local health_bar_label = setting_grid.add({
			type = "label",
	...
	local reset_label = setting_grid.add({
		type = "label",
	...
	::continue::
	local magic_pickup_label = setting_grid.add({
		type = "label",
	...
	if
		...
		and player_modifiers.disabled_modifier[player.index].character_item_pickup_distance_bonus
	then
	...
	local movement_speed_label = setting_grid.add({
		type = "label",
	...
	if
		player_modifiers.disabled_modifier[player.index]
		and player_modifiers.disabled_modifier[player.index].character_running_speed_modifier
	then
	...
	if rpg_extra.enable_stone_path then
		local stone_path_label = setting_grid.add({
			type = "label",
	...
	if rpg_extra.enable_one_punch then
		local one_punch_label = setting_grid.add({
			type = "label",
	...
	if rpg_extra.enable_flame_boots then
		local flame_boots_label = setting_grid.add({
			type = "label",
	...
	if rpg_extra.enable_mana then
		...
		local enable_entity = setting_grid_2.add({
			type = "label",
		...
		local conjure_label = mana_grid.add({
			type = "label",
	...
	if rpg_extra.enable_auto_allocate then
		...
		local allocate_label = allocate_grid.add({
			type = "label",
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/rpg/settings.lua:508')
return Public
