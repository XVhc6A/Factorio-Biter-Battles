...
function get_power_status(teleporter_index, drain_power)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:85')
	local surface = game.surfaces[global.teleporters[teleporter_index].surface]
	local a = {
		left_top = {
			x = global.teleporters[teleporter_index].position.x - 5,
			y = global.teleporters[teleporter_index].position.y - 5,
		},
		right_bottom = {
			x = global.teleporters[teleporter_index].position.x + 6,
			y = global.teleporters[teleporter_index].position.y + 6,
		},
	}
	local power_cells = surface.find_entities_filtered({ area = a, name = "accumulator" })
	if not power_cells[1] then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:99')
		return "No energy source found - Operation not possible"
	end
	if #power_cells < charged_accumulators_required then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:102')
	...
	if #charged_cells < charged_accumulators_required then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:114')
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:121')
	return true
end

local function gui_spawn_new_teleporter(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:124')
	if player.gui.left["spawn_new_teleporter_button"] then
		player.gui.left["spawn_new_teleporter_button"].destroy()
	end
	local b =
		player.gui.left.add({ type = "button", name = "spawn_new_teleporter_button", caption = "Deploy Teleporter" })
...
local function gui_teleporter(player, visited_teleporter_index)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:140')
	...
	local l = t.add({
		type = "label",
		caption = global.teleporters[visited_teleporter_index].name,
		name = visited_teleporter_index,
	})
	...
	if #global.teleporters < 2 then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:168')
		return
	end
	...
	if power_status ~= true then
		...
		frame2.style.top_padding = 14
		frame2.style.bottom_padding = 0
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:176')
		return
	end

	local scroll_pane = frame2.add({
		type = "scroll-pane",
		direction = "vertical",
	...
	for x = #global.teleporters, 1, -1 do
		...
		if x ~= visited_teleporter_index and tile.name == "lab-white" then
			...
			local b = t.add({
				type = "button",
				caption = "> " .. global.teleporters[x].name .. " <",
				name = "teleporter_" .. x,
			})
			...
			local l = tt.add({
				type = "label",
				caption = "X: " .. tostring(global.teleporters[x].position.x) .. "  Y: " .. tostring(
					global.teleporters[x].position.y
				),
			})
			...
			local l = tt.add({
				...
				caption = tostring(
					math.ceil(
						math.sqrt(
							(global.teleporters[x].position.x - player.position.x) ^ 2
								+ (global.teleporters[x].position.y - player.position.y) ^ 2
						),
						0
					)
				) .. " Units",
...
local function spawn_teleporter(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:265')
	...
	if c == 0 then
		...
		if removed_item_count ~= 1 then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:280')
			return
		end
		...
		table.insert(
			global.teleporters,
			{ position = { x = pos.x, y = pos.y }, name = str .. " " .. str2, surface = surface.name }
		)
		local tiles = {
...
local function check_inventory_for_key_item(player_index)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:307')
...
local function on_player_changed_position(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:318')
	if not global.teleporters then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:320')
		return
	end
	local player = game.players[event.player_index]
	if player.character.driving == true then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:324')
		return
	end
	...
	if not tile[1] then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:336')
		return
	end
...
local function on_player_main_inventory_changed(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:346')
	check_inventory_for_key_item(event.player_index)
end

local function on_player_dropped_item(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:350')
	check_inventory_for_key_item(event.player_index)
end

local function on_gui_click(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:354')
	if not event then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:356')
		return
	end
	if not event.element then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:359')
		return
	end
	if not event.element.valid then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:362')
		return
	end
	...
	if string.sub(name, 1, 10) ~= "teleporter" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/teleporters.lua:370')
		return
	end
	...
	if status == true then
		...
		for _, p in pairs(game.connected_players) do
			p.play_sound({
				path = "utility/armor_insert",
				volume_modifier = 1,
				position = global.teleporters[visited_teleporter_index].position,
			})
			p.play_sound({
				path = "utility/armor_insert",
				volume_modifier = 1,
				position = global.teleporters[index].position,
			})
		end
