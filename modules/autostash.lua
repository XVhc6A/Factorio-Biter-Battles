...
local function create_floaty_text(surface, position, name, count)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:22')
...
local function chest_is_valid(chest)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:40')
	if chest.type == "cargo-wagon" then
		...
		if not next(t) then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:57')
			return false, {}
		end

		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:60')
		return true, t
	end
		...
		if e.name ~= "long-handed-inserter" then
			if e.position.x == chest.position.x then
				if e.direction == 0 or e.direction == 4 then
					log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:72')
					return false
				end
			end
			if e.position.y == chest.position.y then
				if e.direction == 2 or e.direction == 6 then
					log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:77')
					return false
				end
			end
		end
	...
	if i1 then
		if i1.direction == 2 or i1.direction == 6 then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:86')
			return false
		end
	end
	...
	if i2 then
		if i2.direction == 2 or i2.direction == 6 then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:92')
			return false
		end
	end
	...
	if i3 then
		if i3.direction == 0 or i3.direction == 4 then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:99')
			return false
		end
	end
	...
	if i4 then
		if i4.direction == 0 or i4.direction == 4 then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:105')
			return false
		end
	end

	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:109')
	return true
end

local function sort_entities_by_distance(position, entities)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:112')
	local t = {}
	local distance
	local index
	local size_of_entities = #entities
	if size_of_entities < 2 then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:118')
		return
	end
...
local function get_nearby_chests(player, a, furnace, wagon)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:139')
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:171')
	return chests
end

local function does_inventory_contain_item_type(inventory, item_subgroup)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:174')
	for name, _ in pairs(inventory.get_contents()) do
		...
		if t and t.subgroup.name == item_subgroup then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:178')
			return true
		end
	end
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:181')
	return false
end

local function insert_item_into_chest(player_inventory, chests, filtered_chests, name, count, furnace, wagon)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:184')
	local container = {
		["container"] = true,
	...
	if furnace then
		for _, chest in pairs(chests) do
			...
			if chest_inventory and chest.type == "furnace" then
				if chest_inventory.can_insert({ name = name, count = count }) then
					...
					if count <= 0 then
						log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:204')
						return
					end
				end
			end
		...
		for _, chest in pairs(chests) do
			if chest.type == "furnace" then
				...
				if amount <= 0 then
					log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:218')
					return
				end
		...
		for _, chest in pairs(chests) do
			if chest.type == "cargo-wagon" then
				...
				if chest_inventory and chest_inventory.can_insert({ name = name, count = count }) then
					...
					count = count - inserted_count
					if count <= 0 then
						log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:241')
						return
					end
				end
			end
		...
		if container[chest.type] then
			...
			if chest_inventory and chest_inventory.can_insert({ name = name, count = count }) then
				if chest_inventory.find_item_stack(name) then
					...
					count = count - inserted_count
					if count <= 0 then
						log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:260')
						return
					end
				end
			end
		...
		if container[chest.type] then
			...
			if chest_inventory and chest_inventory.can_insert({ name = name, count = count }) then
				if chest_inventory.is_empty() then
					...
					count = count - inserted_count
					if count <= 0 then
						log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:278')
						return
					end
				end
			end
			...
			if container[chest.type] then
				...
				if chest_inventory and chest_inventory.can_insert({ name = name, count = count }) then
					if does_inventory_contain_item_type(chest_inventory, item_subgroup) then
						...
						count = count - inserted_count
						if count <= 0 then
							log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:298')
							return
						end
					end
				end
	...
	for _, chest in pairs(filtered_chests) do
		if container[chest.type] then
			...
			if chest_inventory.can_insert({ name = name, count = count }) then
				...
				count = count - inserted_count
				if count <= 0 then
					log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:316')
					return
				end
			end
		end
...
local function auto_stash(player, event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:323')
	local button = event.button
	local ctrl = event.control
	local shift = event.shift
	if not player.character then
		player.print("It seems that you are not in the realm of the living.", print_color)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:329')
		return
	end
	if not player.character.valid then
		player.print("It seems that you are not in the realm of the living.", print_color)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:333')
		return
	end
	local inventory = player.get_inventory(defines.inventory.character_main)
	if inventory.is_empty() then
		player.print("Inventory is empty.", print_color)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:338')
		return
	end
	local chests
	...
	if not chests or not chests[1] then
		...
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:360')
		return
	end
...
local function create_gui_button(player)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:421')
	if player.gui.top.auto_stash then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:423')
		return
	end
	local tooltip
...
local function do_whitelist()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:455')
...
local function on_player_joined_game(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:471')
	create_gui_button(game.players[event.player_index])
end

local function on_gui_click(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:475')
	if not event.element then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:477')
		return
	end
	if not event.element.valid then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:480')
		return
	end
...
function Public.insert_into_furnace(value)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:487')
	if value then
...
function Public.insert_into_wagon(value)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:495')
	if value then
...
Event.on_configuration_changed = function()
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:503')
	do_whitelist()
...
log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/autostash.lua:512')
return Public
