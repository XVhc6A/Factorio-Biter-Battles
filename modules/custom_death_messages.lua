...
local messages = {
	...
	["behemoth-biter"] = {
		" was devoured by a behemoth biter.",
		" was crushed by a behemoth biter.",
		" is biter food.",
	},
	...
	["big-spitter"] = {
		" melted away by acid spit!",
		" couldn't dodge the spit in time.",
		" got blasted away by a spitter.",
	},
	["behemoth-spitter"] = {
		" melted away by acid spit!",
		" couldn't dodge the spit in time.",
	...
	["medium-worm-turret"] = {
		" melted away by acid spit!",
		" couldn't dodge the spit in time.",
		" got blasted away by a medium worm turret.",
	},
	["big-worm-turret"] = {
		" melted away by acid spit!",
		" couldn't dodge the spit in time.",
		" got blasted away by a big worm turret.",
	},
...
local function on_player_died(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:44')
	...
	if event.cause then
		...
		if not cause.name then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:58')
			return
		end
		if messages[cause.name] then
			game.print(player.name .. messages[cause.name][math.random(1, #messages[cause.name])], message_color)
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:62')
			return
		end

		if cause.name == "character" then
			if not player.name then
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:67')
				return
			end
			if not cause.player.name then
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:70')
				return
			end
			if cause.player.tag ~= "" then
				game.print(
					player.name .. tag .. " was killed by " .. cause.player.name .. " " .. cause.player.tag .. ".",
					message_color
				)
			else
				game.print(player.name .. tag .. " was killed by " .. cause.player.name .. ".", message_color)
			end
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:80')
			return
		end
		...
		if cause.type == "car" then
			...
			if driver.player then
				game.print(
					player.name .. tag .. " was killed by " .. driver.player.name .. " " .. player.tag .. ".",
					message_color
				)
				log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:90')
				return
			end
		end

		game.print(player.name .. tag .. " was killed by " .. cause.name .. ".", message_color)
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/custom_death_messages.lua:95')
		return
	end
