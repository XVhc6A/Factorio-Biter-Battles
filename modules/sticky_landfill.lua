...
local function is_position_sticky(surface, position)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:17')
	local tile = surface.get_tile(position)
	if not tile.collides_with("resource-layer") then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:20')
		return
	end
	for _, v in pairs(vectors_2) do
		tile = surface.get_tile({ position.x + v[1], position.y + v[2] })
		if not tile.collides_with("resource-layer") then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:25')
			return true
		end
	end
end
...
local function move_tile(surface, tile_name, position)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:30')
	for key, v in pairs(vectors) do
		...
		if is_position_sticky(surface, p) then
			...
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:35')
			return
		end
	end
end
...
local function safe_players_from_drowning(surface, tiles)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:40')
	...
	log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:51')
	return players
end

local function sticky(surface, tiles, tile_name)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:54')
	local revert_tiles = {}
	...
	for _, placed_tile in pairs(tiles) do
		...
		local resources = surface.find_entities_filtered({
			type = "resource",
			area = {
				{ placed_tile.position.x - 1, placed_tile.position.y - 1 },
				{ placed_tile.position.x + 1, placed_tile.position.y + 1 },
			},
		})
...
local function on_player_built_tile(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:93')
	if event.item.name ~= "landfill" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:95')
		return
	end
...
local function on_robot_built_tile(event)
	log('Func start /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:100')
	if event.item.name ~= "landfill" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/sticky_landfill.lua:102')
		return
	end
