...
event.add(defines.events.on_entity_damaged, function(event)
	...
	if not entity.valid then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/show_health.lua:5')
		return
	end
	if event.entity.name ~= "character" then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/show_health.lua:8')
		return
	end
	local player = game.get_player(event.entity.player)
	if not player.character then
		log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/show_health.lua:12')
		return
	end
	if player.character then
		if player.character.health == nil then
			log('Func ret /Users/drbuttons/git/Factorio-Biter-Battles/modules/show_health.lua:16')
			return
		end
